...
import CV from '../assets/Asiel Valle Valera.pdf'

// Multi language (Format.JS library).
import { FormattedMessage } from 'react-intl'

export default function DownloadCV () {
  // This function is because, when I download my CV, its name change to "Asiel Valle Valera <whatever>.pdf" on Production Mode.
  const handleClickDownloadCV = () => {
    fetch(CV).then((response) => {
      response.blob().then((blod) => {
        ...
        alink.href = fileURL
        alink.download = 'Asiel Valle Valera.pdf'
        alink.click()
      })
    })
  }
